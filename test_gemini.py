...
# --- Configuration ---
video_path = "fifteen_sec_input_720p.mp4"
model_path = "best.pt"  # Assuming 'best.pt' is your trained YOLOv8 model for player detection
...
def processVideo():
    ...
    object_tracker = DeepSort(max_iou_distance=0.6,  # Increased strictness for current frame association
                              max_age=200,             # Increased to allow more frames for re-emergence
                              n_init=10,
                              nms_max_overlap=1.0,
                              max_cosine_distance=0.4, # Cosine distance for appearance similarity
                              nn_budget=None,
                              gating_only_position=False,
                              embedder='mobilenet',   # Use MobileNet for appearance embedding
                              half=True,              # Use half-precision for faster inference if supported
                              bgr=True,
                              embedder_gpu=True)      # Leverage GPU for embedder if available
